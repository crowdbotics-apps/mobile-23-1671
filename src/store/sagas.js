...
function* newplugin231_get__readWorker(action) {
  try {
    const result = yield call(apiService.newplugin231_get__read, action)
    yield put(actions.newplugin231_get__readSucceeded(result))
  } catch (err) {
    yield put(actions.newplugin231_get__readFailed(err))
  }
}
function* newplugin231_get__readWatcher() {
  yield takeEvery(types.NEWPLUGIN231_GET__READ, newplugin231_get__readWorker)
}
